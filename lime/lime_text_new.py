...
class LimeTextExplainer(object):
    ...
    def explain_instance(self,
                         validation_set,                #New parameter
                         validation_data_transformed,   #New parameter
                         text_instance,
                         text_instance_indx,            #New parameter
                         classifier_fn,
                         classifier_pipeline,           #New parameter
                         labels=(1,),
                         top_labels=None,
                         num_features=10,
                         num_samples=5000,
        ...
        Args:
            validation_data_transformed: to generate neighbourhood using Nearest Neighbours technique
        ...
        #Change begins
        nbrs = NearestNeighbors(n_neighbors=num_samples,
                                algorithm='auto',metric=distance_metric).fit(validation_data_transformed)

        dist_nbrs, indices_nbrs = nbrs.kneighbors(validation_data_transformed)

        data = np.asarray(validation_data_transformed[indices_nbrs[text_instance_indx]].todense()) #neighbourhood
        #yss = classifier_fn(validation_set[indices_nbrs[text_instance_indx]]) --Issue3 Fix
        filter_generator = (validation_set[x] for x in indices_nbrs[text_instance_indx])#Issue3 Fix
        validation_set = list(filter_generator)#Issue3 Fix
        yss = classifier_fn(validation_set)#Issue3 Fix

        distances = dist_nbrs[text_instance_indx]
        #Change ends
        ...
        #Change begins
        feature_of_interest = validation_data_transformed[text_instance_indx,:].nonzero()[1]
        #Change ends
        ...
        for label in labels:
            (ret_exp.intercept[label],
             ...
             ret_exp.score, ret_exp.local_pred) = self.base.explain_instance_with_data(
                data[:,feature_of_interest], yss, distances, label, num_features,           # Change - Shortened neighbourhood data array
                model_regressor=model_regressor,
                feature_selection=self.feature_selection)

            #Change begins
            """  Changing ids of words in ret_exp as per the original word. Issue 2 Fix """
        vectorizer= classifier_pipeline.steps[0][1] # refer yss calculation

        for pos,loc_exp in ret_exp.local_exp.items():
            updated_loc_exp = []
            for t in loc_exp:
                word = vectorizer.get_feature_names()[feature_of_interest[t[0]]]
                i = indexed_string.inverse_vocab.index(word)
                updated_loc_exp.append((i,t[1]))
            ret_exp.local_exp[pos]=updated_loc_exp
            #ret_exp.local_exp
            #Change ends
        return ret_exp
