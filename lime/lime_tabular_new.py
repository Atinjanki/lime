...
class LimeTabularExplainer(object):
    ...
    def explain_instance(self,
        ...
        nbrs = NearestNeighbors(n_neighbors=num_samples,
                                algorithm='auto', metric=distance_metric).fit(validation_set)
        dist_nbrs, indices_nbrs = nbrs.kneighbors(validation_set)

        inverse = np.asarray(validation_set[indices_nbrs[text_instance_indx]]) #neighbourhood

        scaler = StandardScaler()
        #scaled_data = (data - self.scaler.mean_) / self.scaler.scale_
        scaled_data = scaler.fit_transform(inverse)
        ...
        if self.discretizer is not None:
            ...
            categorical_features = range(inverse.shape[1])
            #Change ends
        ...
        domain_mapper = TableDomainMapper(feature_names,
                                          values,
                                         # scaled_data[0],  #Change begins
                                          inverse[0],       #Change ends
